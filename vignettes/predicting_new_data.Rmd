...
params: 
  input_dir: 
    ...
    value: C:/Work/data/predicting_input/
    input: text
---
```{r, include = FALSE}
...
output_dir <- paste0(dirname(input_dir), "/prediction_tifs/")
...
mask_range <- t(sapply(sample_pnts[, c("gradient", "mean_curv", "ln_pca_k1_48")], range))
colnames(mask_range) <- c("min", "max")
rownames(mask_range) <- c("gradient", "mean_curv", "pca")
