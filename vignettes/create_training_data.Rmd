...
data_path <- system.file("data", package = "TerrainWorksUtils")
elev <- rast(file.path(data_path, "elev_scottsburg.tif"))
...
analysisRegionMask <- createAnalysisRegionMask(
  varsRaster, 
  initiation_points, 
  maskVars = c("grad_15", "plan_15", "prof_15"), 
  expansionFactor = 1
)
...
plot(elev)
points(trainingPoints, col = ifelse(trainingPoints$class == "positive", 
                                    "black", 
                                    "red")
         )
```
