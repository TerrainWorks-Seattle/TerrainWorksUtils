...
# It can take a long time to create them, so if they're already there, read them in.
  ...
  ls_pnts <- rbind(ls_pnts,  terra::extract(derivs,
                              initiation_points,
                              method = "bilinear"))
...
for (i in 1:num_files) {
  ...
  for (j in 1:ncol(durations)) {
    ...
    covariates <- c(covariates, contributing_area(raster = paste0(output_dir, raster_name),
                                                    dem = DEM, 
                                                    length_scale = length_scale, 
                                                    k = k, 
                                                    d = d, 
  ...
  ls_pnts <- rbind(ls_pnts, terra::extract(covariates,
                                           initiation_points,
                                           method = "bilinear"))
...
# Set values within the inner buffer and beyond the outer buffer to nodata
  analysis_mask <- mask_by_range(covariates[[1:2]], range_mx)
  ...
  nonls_pnts <- rbind(nonls_pnts,  terra::extract(covariates,
                                                  neg_points,
                                                  method = "bilinear"))
