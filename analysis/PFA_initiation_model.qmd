...
Let's do the same thing, but leave out stand age and distance to road. We are interested in how these variables affect the landslide initiation probability, but don't want to include them in the final output for
...
The output of these filters has been scaled and combined in the figure below. Each different method showed relatively similar ordering of the variables. Of note is that the longer durations for the partial contributing areas are consistently higher
...
pca_rast <-  contributing_area(raster = paste0(dir, basin, "/pca_6_1.flt"))
...
Here we predict on a sub-basin of the larger Umpqua watershed. There are 9 landslide initiation points located inside this basin.
...
To generate a success rate curve, we combine multiple basins. The success rate curve is the proportions of landslide predicted over the proportion of the predicted area. A smaller area under the success rate curve indicates that the model was more specific with the areas that it deemed to have a high probability of landslide initiation. Looking at the model performance in this way limits the
