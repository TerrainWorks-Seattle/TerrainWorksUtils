...
params: 
  ...
  dem_dir: 
    label: Data Directory
    value: /Users/julialober/Documents/terrainworks/code/sandbox/data/downloaded_3.6/in/
    input: text
  pred_dir: 
    label: Data Directory
...
```{r}
set.seed(12345)
ls_task$select(c("ln_pca_k1_48", "mean_curv", "gradient"))
...
log_reg <- lrn("classif.log_reg", 
                predict_type = "prob")
log_reg$train(task) # train (calibrate) the model on the entire data set

# set up a data frame with just the features
x = sample_pnts[, training_cols]

# create a new instance of an iml Predictor object for this model
model = iml::Predictor$new(log_reg, data = x, y = sample_pnts$class)
...
importance$plot(features = num_features) + 
  labs(title = "Permutation Feature Importance", 
       subtitle = "For predicting landslide initiation")
```
...
topo_files <- c(paste0("GRADIENT,", pred_dir, "Umpqua/basin_1/gradient.flt"), 
                paste0("MEAN CURVATURE,", pred_dir, "Umpqua/basin_1/mean_curv.flt"))
...
pd$ln_pca_k1_48 <- (pd$ln_pca_k1_48 - center_vals[2]) / scale_vals[2]
pd$mean_curv <- (pd$mean_curv - center_vals[3]) / scale_vals[3]

pd <- subset(pd, select = c("x", "y", "mean_curv", "gradient", "ln_pca_k1_48"))
```
...
```{r}
basin_list <- list.files(params$dem_dir, paste0("^basin_[1-9]{1,3}\\.flt"), recursive = TRUE)
# basin_list <- list.dirs(pred_dir, recursive = TRUE, full.names = FALSE)
predict = TRUE
```
...
if (predict) {
  ...
  topo_files <- c(paste0("GRADIENT,", pred_dir, basin, "/gradient.flt"), 
                paste0("MEAN CURVATURE,", pred_dir, basin, "/mean_curv.flt"))
  print(topo_files)
  topo_rast <- elev_deriv(rasters = topo_files)
...
```{r}
load(paste0(pred_dir, "grad_pca_gbyp.Rdata"))
success_curve_2 <- cbind(bins, cumul_prop)

ggplot(success_curve) + 
  geom_line(aes(x = area_prop, y = prob_prop), color = "red") + 
  geom_point(aes(x = area_prop, y = prob_prop), color = "red") + 
  geom_line(data = success_curve_2, aes(x = area_prop, y = prob_prop), color = "blue") + 
