...
for (i in 1:num_files) {
  ...
  covariates <- terra::rast(paste0(file_name, ".flt"))
...
# Get distance-to-road values
  ...
  if (nrow(roads) == 0) {
    distTo <- rast(covariates,
                   nlyr = 1)
    values(distTo) <- NA
    covariates <- c(covariates, distTo)
  ...
  for (j in 1:ncol(durations)) {
    ...
    covariates <- c(covariates, 
                    TerrainWorksUtils::contributing_area(raster = file_name,
                                                         ...
                                                         d = d, 
                                                         scratch_dir = output_dir))
    names(covariates[[4+j]]) <- raster_name
...
save(ls_1996, file = paste0(output_dir, "ls_1996.Rdata"))
save(ls_2007, file = paste0(output_dir, "ls_2007.Rdata"))
save(ls_2011, file = paste0(output_dir, "ls_2011.Rdata"))
save(nonls_1996, file = paste0(output_dir, "nonls_1996.Rdata"))
save(nonls_2007, file = paste0(output_dir, "nonls_2007.Rdata"))
save(nonls_2011, file = paste0(output_dir, "nonls_2011.Rdata"))
```
