...
params:
  output_dir:
    ...
    value: D:/Google Drive/DataFiles/tempDir/out/
    input: text
  data_dir:
    label: Data Directory
    value: D:/Google Drive/DataFiles/tempDir/in/
    input: text
  length_scale:
  ...
  input_list:
    ...
    value: D:/Google Drive/DataFiles/tempDir/in/DEMs/input_list.txt
    input: text
  inner_buffer:
...
Currently, this code produces a list of files in tempDir/out that is numbered in order of the basins as they come in input_file.txt.

We want code that makes it easy to transition this list of files into a basin/subbasin/files structure (like Julia is currently working with), or a basin/files (like the tempDir/in looks) structure, or even just renaming to more descriptive files. In any of these cases, we just need to reference the list to make sure that we know from which basin it comes, and then sort it into the appropriate structure. I may even turn this into a function, for internal TerrainWorks use.

Here is the full list of basins and initiation point files that we have in the tempDir/in folder. This shows the structure that we use here.

```{r}
DEM <- list.files(params$data_dir,
             "^basin.*\\.flt$",
             recursive = TRUE,
             full.names = TRUE)
init_pts <- list.files(params$data_dir,
             "^init_pnts.*\\.shp$",
             recursive = TRUE,
             full.names = TRUE)
  
data_files <- as_tibble(cbind(DEM, init_pts))
data_files
```

This files generates the following attributes for each basin:
...
All as .flt files. Next up will be to write code to convert the .flt files into .tif files which will be much smaller and a bit more universal.

Note this code relies heavily on regular expressions. It would be useful to have a solid understanding of how those work before diving in to trying to understand this code.

```{r}
attr <- c("accumMax", "distTo", "gradient", "meancurv", "pca_48", "profcurv", "tancurv")
attr_snake <- c("accum_max", "dist_to", "gradient", "mean_curv", "pca", "prof_curv", "tan_curv")
attr_regex <- paste0("^", attr, ".*\\..*$")

# find the files in the output directory
old_fns <- sapply(attr_regex, list.files, path = params$output_dir, recursive = TRUE, full.names = TRUE)
names(old_fns) <- attr_snake

# leave out dist_to for now, since we are missing a couple of files 
use <- c(1, 3, 4, 5, 6, 7)
old_fns <- old_fns[use]
```

```{r}
# can be "basin-files", "verbose-filenames", or "basin-subbasin"
output_style <- "basin-files"

# get the order of the filenames from input_file.txt 
fn_order <- as_tibble(data_files$DEM)
names(fn_order) <- c("old_path")
fn_order$basin_id <- seq(1:length(fn_order$old_path))
fn_table <- table(dirname(data_files$DEM))

# change filenames to snake case
new_fns <- lapply(old_fns, basename)
for (i in 1:length(new_fns)) {
  new_fns[[i]] <- gsub(attr[use[i]], attr_snake[use[i]], new_fns[[i]])
}
new_fns <- lapply(new_fns, as_tibble)
new_fns <- lapply(new_fns, function(x) {
  x$basin_id <- as.numeric(gsub("\\D", "", x$value))
  return(x)
})
```

Now we have two tibbles, both with a column called basin_id that should match up to each other.

```{r}
if (output_style == "basin-files") {
  fn_order$new_path <- dirname(fn_order$old_path)
  fn_order$new_path <- gsub("^.*/in", paste0(gsub("/tempDir.*$", "", params$output_dir), "/covariates"), fn_order$new_path)
  
  res <- lapply(fn_order$new_path, dir.create, recursive = TRUE, showWarnings = FALSE)
  
  joined <- lapply(new_fns, left_join, y = fn_order, by = "basin_id")
  joined <- lapply(joined, function(x) {
    x$new <- unite(x, 'new', all_of(c("new_path", "value")), sep = "/", remove = FALSE)
  })
...
} else if (output_style == "verbose-filenames") {
  
} else if (output_style == "basin-subbasin") {
}
```

Now, to convert, we just run these two lines of code:

```{r}
convert <- list.files("D:/Google Drive/DataFiles/covariates", ".flt$", recursive = TRUE, full.names = TRUE)
lapply(convert, flt_to_tif, rm_old = TRUE)
```
