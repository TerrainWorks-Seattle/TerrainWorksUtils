...
# Get the initiation points for this DEM for each time slot (1996, 2007, 2011)
  ...
  if (nrow(pnts_2007) > 0) {
    print(paste0("Number of 2007 points =", nrow(pnts_2007)))
...
# Get distance-to-road values
  ...
  covariates <- c(covariates,
                  TerrainWorksUtils::distance_to_road(raster = paste0(output_dir, raster_name),
                                                      dem = DEM,
                                                      road_shapefile = road_shapefile,
                                                      radius = 1000.,
                                                      scratch_dir = output_dir))
  names(covariates[[3]]) <- "dist_to_road"
  ...
  for (j in 1:ncol(durations)) {
    ...
    covariates <- c(covariates, 
                    TerrainWorksUtils::contributing_area(raster = paste0(output_dir, raster_name),
                                                         ...
                                                         d = d, 
                                                         scratch_dir = output_dir))
    names(covariates[[3+j]]) <- raster_name
    
  }
  
  if (nrow(pnts_1996) > 0) {
    print(paste0("Number of points in 1996 is ", nrow(pnts_1996)))
  ...
  # Set values within the inner buffer and beyond the outer buffer to nodata
    ...
    print(paste0("Number of nonlandslide points = ",num_points))
    ...
    print("Got nonls_1996 for this iteration")
  }
  ...
  # Set values within the inner buffer and beyond the outer buffer to nodata
    ...
    print("Got nonls_2007 for this iteration")    
  }
  ...
  # Set values within the inner buffer and beyond the outer buffer to nodata
    ...
    print("Got nonls_2011 for this iteration")     
  }  
...
save(ls_pnts, file = paste0(output_dir, "ls_pnts.Rdata"))
save(nonls_pnts, file = paste0(output_dir, "nonls_pnts.Rdata"))
