...
attr_snake <- c("accum_max", "dist_to", "gradient", "mean_curv", "pca_48", "prof_curv", "tan_curv")
...
for (i in 1:length(new_fns)) {
  new_fns[[i]] <- gsub(attr[i], attr_snake[i], new_fns[[i]])
}

if (output_style == "basin-files") {
  new_path <- gsub("basin.*$", "", fn_order)
  new_path <- gsub("^.*/in", paste0(gsub("/tempDir.*$", "", params$output_dir), "/covars"), new_path)
  
  lapply(new_path, dir.create, recursive = TRUE, showWarnings = FALSE)
  
  for (i in 1:length(new_fns)) {
    new_fns[[i]] <- paste0(new_path, new_fns[[i]])
  }
  
  mapply(file.copy, old_fns, new_fns)
  
} else if (output_style == "verbose-filenames") {
  new_prefix <- gsub("/basin.*$", "", fn_order)
  new_prefix <- gsub("^.*/in", "D:/Google Drive/DataFiles/covars-verbose", new_prefix)
  ...
  new_prefix <- gsub("/basin.*$", "", fn_order)
  new_prefix <- gsub("^.*/in", "D:/Google Drive/DataFiles/covars", new_prefix)
}
```
