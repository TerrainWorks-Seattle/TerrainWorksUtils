...
In @fig-EmpiricalSurvivalCurves and the following histograms we showed that the cumulative distribution of measured runout lengths in the DOGAMI inventory is highly right (positively) skewed, consistent with what Miller and Burnett [-@miller2008] found with the 1996 storm data. Miller and Burnett fit an exponential distribution to those data; here we show fits for both an exponential and a log-normal distribution.
...
plot_df <- rbind(exp_df, lognorm_df)

ggplot(data = tracks, aes(x = Tend, y = 1 - ..y.., fill = "DOGAMI")) + labs(fill="Data") +
  stat_ecdf(geom = "point", shape = 21, size = 4, alpha = 0.5, pad = FALSE, color="black") +
  coord_cartesian(xlim = c(0,2000)) +
  ...
  geom_line(data=plot_df, aes(x = Length, y = Surv, color = Model), linewidth=1.2) +
  scale_color_manual(values=c("blue", "red4"))
```

The log normal distribution is clearly a better fit.
hazard
