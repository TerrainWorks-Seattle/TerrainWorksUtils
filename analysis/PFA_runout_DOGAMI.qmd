...
where $l_I$ and $w_I$ are the average length and width of initiating landslides and $d$ is the soil depth at the initiation site based on Equations @eq-meanDepth and @eq-scourVol above.
...
Following Miller and Burnett [-@miller2008], we use the distribution of volume ratios ($V_D/V_S$) for debris-flow tracks in the inventory to set the constant of proportionality. The volume ratios calculated for The value of that constant shifts the distribution of modeled volume ratios Miller and Burnett set that constant so that the median volume ratio calculated for runout tracks in the 1996 Storm inventory had a value of one; when the modeled deposited volume equaled the modeled scoured volume. Here we set the constant so that 95% of the runout tracks have calculated volume ratios of one or less. We will use estimates of deposited volume to characterize debris-flow source areas (the initiation zones) in terms of sediment flux to the fish-bearing channel network. There cannot be a greater volume deposited than scoured.
...
The intention is to separate rock types into groups that influence landslide initiation and runout processes differently. The attributes that might distinguish such groups include degree of fracturing, rate of weathering, and presence of bedding or foliation. I have taken a broad initial classification - 12 classes to describe all rock types - and made it even broader: just five classes. The three ODF 1996 Storm study sites that provided data used for calibrating the probability of scour and deposition do not span a large geographic extent and will miss most of the initial 12 classes. The final five classes divide rock types by modes of origination. Other classifications could be used, but this proved sufficient to resolve differences in scour and deposition probabilities between rock-type classes.
...
![](images/1996_Storm_Study_Sites.png){fig-align="center" width="6in"}

Surveys at these three areas including detailed mapping of channel impacts associated with the storm. Being field based, these surveys were able to distinguish four types of debris-flow-runout behavior: 1) a scoured debris path, along unchannelized topography or within channels greater than 40% gradient, 2) channelized flow with scour, 3) channelized flow with deposition, and 4) transitional channelized flow with no net scour or deposition. At these sites, all channels with gradients up to 40% were surveyed and storm impacts were categorized as high, medium, or low and recorded at \~150-foot increments [@robison1999]. High impacts were associated with "debris torrents" and impacts in these channels were further categorized as scour, deposit, transitional, or debris jam. Although "debris torrent" and "debris flow" are not synonomous, we used this mapping to calibrate a logistic regression model for the probability of debris-flow scour or deposition.
...
# Topographic data extracted by Fortran program DF_track_ODF
...
# Rock type from classification in above table performed in ArcGIS
...
The range of measured gradients across the three sites is relatively consistent, shown in the box plots below for scour and depositional zones. Mapleton has slightly higher gradients for both cases, Vida slightly lower, and Tillamook inbetween. These three sites overly different substrates. Mapleton is in marine sandstones, Tillamook in volcanics, and Vida in volcaniclastic rocks, so perhaps the slight differences in the distribution of gradients between sites reflects differences in underlying rock type. Scour gradients are consistently higher than depositional gradients, consistent with other studies.
...
Tangential curvature is a measure of topographic confinement measured perpendicular to the downslope direction. I have set the sign on curvature so that positive values indicate concave up (swales, channels) and negative values indicate concave down (noses). Here are box plots for tangential curvature:
...
Tangential curvatures are almost all positive, suggesting that these tracks are predominately in channels or swales, and values for depositional zones are consistently lower than for scour, indicating scour in more confined channels and deposition in less confined channels. Except for Mapleton, where tangential curvature through zones mapped as depositional appear no different than though zones mapped as scour. This is distinctly different than seen for Tillamook and Vida.

All sites experience deposition over the same range of tangential curvatures, but Mapleton has a much larger proportion of the depositional zones in more confined channels. The maps shown in Figures @fig-TillamookData, @fig-VidaData, and @fig-MapletonData show that channel impacts were mapped much further downstream at the Tillamook and Vida sites than at the Mapleton site. The box plots below show how drainage area was distributed across the channels mapped with debris-torrent impacts at the three sites.
...
Scour zones occupied about the same range of channel sizes (with a few larger outliers for Tillamook and Vida), but depositional zones were mapped through much larger channels at Tillamook and Vida than at Mapleton. As described in Robison [-@robison1999], field crews were mapping "debris-torrent" impacts. These may have included channel impacts associated with dam-break floods [@coho1993], migrating organic dams [@johnson1991], sediment waves [@miller2000], or hyperconcentrated flows [e.g., @jakob2007], which would extend further downstream than a debris-flow deposit. Our focus here is on debris flows, because debris flows are the process that carries sediment and large wood from upslope areas and headwater channels to fish-bearing streams. These other processes impact fish-bearing streams, but they are generally a consequence of a debris flow that deposits into a larger channel and the factors that affect the extent of these types of channel impacts are different than those that affect debris-flow runout. Lacking any field-based alternative, we use this dataset, but recognize that the calibrated model for probability of scour and deposition may be biased by the presence of channels that are probably downstream of actual debris-flow deposits.
...
These three sites fall almost entirely in different rock types. Mapleton is in sedimentary rocks, Tillamook in volcanic, and Vida in volcaniclastic.
...
ggplot(data=all_sites) + 
  ...
  labs(title='Rock types encountered at the three study sites',y='Proportion of impacted channel length') +
...
grad <- effect.data$Gradient
gradEffects <- ggplot(data=grad, aes(x=.borders, y=.value, col=.class)) +
  geom_line(size=2) +
  labs(x="Gradient", y="Effect")

gradEffects + scale_color_discrete(name="Track Type", labels=c("Deposit", "Scour", "Transitional"), values=cbbPalette)
```
