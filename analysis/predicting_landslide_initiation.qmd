...
```{r}
predicting_data$pca_k1_48 <- log(predicting_data$pca_k1_48, base = 2)

#pre process the data: take the log of the pca and center+scale the data
# predict_task <- as_task_classif_st(predicting_data)
predicting_data_eng <- pl_feature_eng$predict_newdata(predicting_data)

predictions_df <- logreg_learner$predict_newdata(predicting_data_eng)
```
