...
pts_cumul$ls.prop = seq(1, 9) / total_pts
...
A note: this code assumes we already have a trained model to use for predictions. The learner we are using here comes from the code above.
...
for (basin in basin_list) { 
  ...
  topo_files <- c(paste0("GRADIENT,", basin, "gradient.flt"),
                paste0("MEAN CURVATURE,", basin, "mean_curv.flt"))
  topo_rast <- elev_deriv(rasters = topo_files)
  pca_rast <-  contributing_area(raster = paste0(basin, "pca_k1_d48.flt"))
  rasters <- c(topo_rast, pca_rast)
