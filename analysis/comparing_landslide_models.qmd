...
landslide_task_noage <- as_task_classif_st(x = subset(sample_pnts, select = -c(age, dist_to_road)), 
                                    id = "landslide_initiation", 
                                    target = "class", 
                                    positive = "pos", 
                                    coordinate_names = c("x", "y"), 
...
pl_fselect$train(landslide_task_noage)
...
# feature_set <- c("mean_curv", "pca_k1_48")
...
Now that I am using the scaled version of the data, I agree. The pca value has the largest coefficient, and is consistently the variable that is chosen first during the feature selection, indicating that it is a highly explanatory variable for our dataset.
